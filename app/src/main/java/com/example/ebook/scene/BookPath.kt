...
/**
 * Describes how the 3D book moves along its showcase path.
 *
 * Progress runs `0f → 1f` and the book slides straight along X, from [startX] at `t = 0` to [endX]
 * at `t = 1`. No curve, no turning.
 *
 * Both ends are in the book's local units (the model is scaled to 1.3 units — see [Book3D]),
 * relative to the book's base position. Negative is left, positive is right.
 */
data class BookPath(
